# ## generador faker (limites:481M/16ram)
...
### generador temporal usando extractos del anterior :3
from faker import Faker
import random
import csv
...
TAMANIOS = {
    "Pequeña": (10000, 200000),
    "Mediana": (200000, 2000000),
    "Grande": (2000000, 10000000)
}

INDUSTRIAS = ["Tecnología","Finanzas","Retail","Manufactura","Salud","Educación"]

EVENTOS = [
    ("expansion","Alto"),
    ("despidos","Medio"),
    ("nueva inversion","Alto"),
    ("problema legal","Alto"),
    ("crecimiento ventas","Medio")
]

def generar_cliente(id_cliente):
    tamaño = random.choice(list(TAMANIOS.keys()))
    ingresos = random.randint(*TAMANIOS[tamaño])
    ...
    return {
        ...
        "presupuesto_TI": int(ingresos * random.uniform(0.02,0.12)),
        "interes_producto": random.choice(["Alto","Medio","Bajo"]),
        "region": fake.state()
    }

def generar_eventos(id_cliente):
    eventos = []
    for _ in range(random.randint(1,MAX_EVENTOS)):
        evento, impacto = random.choice(EVENTOS)
        eventos.append({
            "id_cliente": id_cliente,
...
def generar_historial(id_cliente):
    return {
        "id_cliente": id_cliente,
        "contactos_previos": random.randint(0,10),
        "compras_previas": random.randint(0,5),
        "ultima_interaccion": fake.date_between(start_date='-1y', end_date='today'),
        "respuesta_email": random.choice(["Positiva","Negativa","Neutral"]),
        "churn_riesgo": random.choice(["Alto","Medio","Bajo"])
    }
...
for i in range(1, NUM_CLIENTES+1):
    c = generar_cliente(i)
    clientes.append(c)
    historial.append(generar_historial(i))
    eventos.extend(generar_eventos(i))
...
guardar_csv("clientes.csv", clientes)
guardar_csv("eventos.csv", eventos)
guardar_csv("historial.csv", historial)

print("Datasets generados chidos C:.")
